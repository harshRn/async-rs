...
impl Executor {
        ...
        F: Future<Output = ()> + 'static,
    {
        ...
        println!("polling while future data is on stack");
        // match future.as_mut().poll(&waker) {
        //     PollState::NotReady => {
        //         println!("future was not ready so we spawn it on the executor");
        //     }
        ...
        loop {
            while let Some(id) = self.pop_ready() {
                ...
                let waker = self.get_waker(id).into();
            ...
            if task_count > 0 {
                ...
                thread::park(); // efficient and does not do extra work, but executor is depndent on polling
            } else {
                println!("{name}: All tasks are finished");
                break;
            }
            println!("unparked here");
        }
    }
}
